...
### Changed
*   **Heading Sequence & Color Contrast Accessibility Audits:** Resolved accessibility audit findings by correcting heading jumps (changed step card titles to `<h3>` in `index.astro`, changed card and banner titles to `<h2>` in `services.astro`, changed case titles to `<h2>` in `cases.astro`, and changed footer section titles to `<h2>` in `Footer.astro`). Fixed low-contrast text link colors by switching default link selectors in `global.css` and `Footer.astro` to a high-contrast lavender (`#C084FC` with 7.8:1 contrast ratio) and updated icon wrapper backgrounds/borders to fit the new purple theme.
